...
class YoloModel:
    ...
    def resize_image(self, src_img, keep_ratio=True):
        """
        Resize image method
        :param src_img:
        :param keep_ratio:
        ...
        if keep_ratio and src_img.shape[0] != src_img.shape[1]:
            ...
            if hw_scale > 1:
                ...
                img = cv2.resize(src_img, (neww, newh), interpolation=cv2.INTER_AREA)
                left = int((self.input_width - neww) * 0.5)
                img = cv2.copyMakeBorder(
                    img, 0, 0, left, self.input_width - neww - left, cv2.BORDER_CONSTANT, value=(0, 0, 0)
                )  # add border
            else:
                ...
                img = cv2.copyMakeBorder(
                    img, top, self.input_height - newh - top, 0, 0, cv2.BORDER_CONSTANT, value=(0, 0, 0)
                )
        else:
            img = cv2.resize(src_img, (self.input_width, self.input_height), interpolation=cv2.INTER_AREA)
    ...
    def detect(self, src_img):
        ...
        :return:
        """
        input_img, newh, neww, padh, padw = self.resize_image(cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB))
        scale_h, scale_w = src_img.shape[0] / newh, src_img.shape[1] / neww
    ...
    def post_process(self, preds, scale_h, scale_w, padh, padw):
        ...
        for i, pred in enumerate(preds):
            ...
            bbox = (
                self.distance2bbox(self.anchors[stride], bbox_pred, max_shape=(self.input_height, self.input_width))
                * stride
            )
        ...
        bboxes_wh[:, 2:4] = bboxes[:, 2:4] - bboxes[:, 0:2]  # using xywh coordinates
        classIds = np.argmax(scores, axis=1)
        confidences = np.max(scores, axis=1)  # gettings max class prob/conf
    ...
    def draw_detections(self, image, boxes, scores, kpts):
        ...
        for box, score, kp in zip(boxes, scores, kpts):
            ...
            # Draw rectangle
            cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 255), thickness=3)
