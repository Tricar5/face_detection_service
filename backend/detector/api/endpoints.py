...
@api_router.post("/face/process", response_model=list[TaskResponse], status_code=status.HTTP_201_CREATED)
...
@api_router.get("/face/{task_id}", status_code=status.HTTP_200_OK, response_model=TaskResponse)
...
async def get_task_data(task_id: str, r=Depends(get_redis)):
